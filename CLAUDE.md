...
- **Sub-field 7 structure (confirmed)** — sub-field 7 of the state response is a nested protobuf message with two inner fields: inner field 1 is the **charging indicator** (0 = not charging, 3 = charging observed). The value 3 was previously misread as "countdown running" but is present on the confirmed-charging device in both on and off states, and absent (0) on the non-charging device in all states. Inner field 2 is the battery level enum (3 = 25 %, 4 = 50 %, 5 = 75 %, 6 = 100 %).
